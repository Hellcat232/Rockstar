import{r as e,s as g,a as w,b as L}from"./assets/check-page-30e88e65.js";import{a as l,b as T,i as $}from"./assets/vendor-cd34f230.js";const d="https://books-backend.p.goit.global/books/",p={"Content-Type":"application/json","Cache-Control":"max-age=1300"};async function E(){const t="category-list";try{const o=`${d}${t}`;return(await l.get(o,{headers:p})).data}catch(o){console.log(o)}}async function B(){const t="top-books";try{const o=`${d}${t}`;return(await l.get(o,{headers:p})).data}catch(o){console.log(o)}}async function m(t){const o=`category?category=${t}`;try{const s=`${d}${o}`;return(await l.get(s,{headers:p})).data}catch(s){console.log(s)}}function C(t){return t.map(o=>`<li class="sidebar-categories-item"><a class="sidebar-category-link">
   ${o.list_name}
   </a>
  </li>`).join(`
`)}function S(t){const o=C(t);e.categories.insertAdjacentHTML("beforeend",o)}function _(t){return t.map(({list_name:o,books:s})=>{let i;return window.innerWidth>=1440?i=c(s,o):window.innerWidth>=768?i=c(s.slice(0,3),o):i=c(s.slice(0,1),o),`
      ...
      `}).join(`
`)}function x(t){const o=_(t);e.bestBooks.insertAdjacentHTML("beforeend",o)}function c(t,o){return t.filter(({list_name:s})=>s===o).map(({author:s,list_name:i,book_image:a,title:n,_id:y})=>`
...
`)}
  ...
  </h1>`,e.categoryTitle.innerHTML=o;else{const s=t.split(" "),i=s.slice(-1);o=`<h1 class="best-books-title">${s.slice(0,-1).join(" ")} <span class="best-books-title-colour">${i.join(" ")}</span></h1>`,e.categoryTitle.innerHTML=o}}function u(t){t.forEach(o=>{const s=document.getElementById(o._id);s&&s.addEventListener("click",i=>{i.preventDefault(),console.log(`${o._id} clicked`);const a=T.create(`
          <div class="modal">
