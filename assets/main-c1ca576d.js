import{a as l}from"./vendor-0cb09735.js";async function y(){const s="https://books-backend.p.goit.global/books/"+"category-list";return(await l.get(s)).data}async function h(){const s="https://books-backend.p.goit.global/books/"+"top-books";return(await l.get(s)).data}async function b(e){const t="https://books-backend.p.goit.global/books/",s=`category?category=${e}`,r=t+s;return(await l.get(r)).data}function p(e){return e.map(t=>`<li class="sidebar-categories-item"><a class="sidebar-category-link">
   ${t.list_name}
   </a>
  </li>`).join(`
`)}function f(e){const t=p(e);o.categories.insertAdjacentHTML("beforeend",t)}function v(e){return e.map(({list_name:t,books:s})=>{let r;return window.innerWidth>=1440?r=n(s,t):window.innerWidth>=768?r=n(s.slice(0,3),t):r=n(s.slice(0,1),t),`
      ...
      `}).join(`
`)}function B(e){const t=v(e);o.bestBooks.insertAdjacentHTML("beforeend",t)}function n(e,t){return e.filter(({list_name:s})=>s===t).map(({author:s,list_name:r,book_image:a,title:c,_id:m})=>`
